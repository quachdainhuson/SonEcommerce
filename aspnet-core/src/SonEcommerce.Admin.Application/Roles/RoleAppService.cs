...
namespace SonEcommerce.Admin.Roles
{
        ...
        CreateUpdateRoleDto>, IRolesAppService
    {
        ...
        {
            var query = await Repository.GetQueryableAsync();
            var isNameExist = query.Any(x => x.Name == input.Name);
            if (isNameExist)
            {
                throw new BusinessException(SonEcommerceDomainErrorCodes.RoleNameAlreadyExists).WithData("Name", input.Name);
            }
        ...
        public override async Task<RoleDto> UpdateAsync(Guid id, CreateUpdateRoleDto input)
        {
            ...
            var query = await Repository.GetQueryableAsync();
            var isNameExist = query.Any(x => x.Name == input.Name && x.Id != id);
            if (isNameExist)
            {
                throw new BusinessException(SonEcommerceDomainErrorCodes.RoleNameAlreadyExists).WithData("Name", input.Name);
            }
        ...
        public async Task<GetPermissionListResultDto> GetPermissionsAsync(string providerName, string providerKey)
        {
            ...
            var groups = await PermissionDefinitionManager.GetGroupsAsync();
                foreach (var group in groups.Where(x => x.Name.StartsWith("AbpIdentity") || x.Name.StartsWith("SonEcomAdmin")))
            {
