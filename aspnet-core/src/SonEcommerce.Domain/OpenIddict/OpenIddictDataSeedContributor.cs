...
namespace SonEcommerce.OpenIddict;
...
public class OpenIddictDataSeedContributor : IDataSeedContributor, ITransientDependency
{
    ...
    {
        if (await _openIddictScopeRepository.FindByNameAsync("SonEcommerce") == null)
        {
            await _scopeManager.CreateAsync(new OpenIddictScopeDescriptor {
                Name = "SonEcommerce", DisplayName = "SonEcommerce API", Resources = { "SonEcommerce" }
            });
        }

        if (await _openIddictScopeRepository.FindByNameAsync("SonEcommerce.Admin") == null)
        {
            await _scopeManager.CreateAsync(new OpenIddictScopeDescriptor
            {
                Name = "SonEcommerce.Admin",
                DisplayName = "SonEcommerce Admin API",
                Resources = { "SonEcommerce.Admin" }
            });
        }
    }
    ...
    private async Task CreateApplicationsAsync()
    {
        ...
        adminScope.Add("SonEcommerce.Admin");

        var clientScope = new List<string>();
        clientScope.AddRange(commonScopes);
        clientScope.Add("SonEcommerce");

        var configurationSection = _configuration.GetSection("OpenIddict:Applications");

        //Admin Web Client
        var webAdminClientId = configurationSection["SonEcommerce_Admin:ClientId"];
        if (!webAdminClientId.IsNullOrWhiteSpace()) 
        {
            var adminWebClientRootUrl = configurationSection["SonEcommerce_Admin:RootUrl"].TrimEnd('/');
            await CreateApplicationAsync(
                name: webAdminClientId,
                type: OpenIddictConstants.ClientTypes.Confidential,
                consentType: OpenIddictConstants.ConsentTypes.Implicit,
                displayName: "Admin Application",
                secret: configurationSection["SonEcommerce_Admin:ClientSecret"] ?? "1q2w3e*",
        ...
        // Client Application
        var webClientId = configurationSection["SonEcommerce_Web:ClientId"];
        if (!webClientId.IsNullOrWhiteSpace())
        {
            var webClientRootUrl = configurationSection["SonEcommerce_Web:RootUrl"].EnsureEndsWith('/');
            await CreateApplicationAsync(
                name: webClientId,
                type: OpenIddictConstants.ClientTypes.Confidential,
                consentType: OpenIddictConstants.ConsentTypes.Implicit,
                displayName: "Console Test / Angular Application",
                secret: configurationSection["SonEcommerce_Web:ClientSecret"] ?? "1q2w3e*",
                grantTypes: new List<string>
                {
        ...
        var swaggerClientId = configurationSection["SonEcommerce_Admin_Swagger:ClientId"];
        if (!swaggerClientId.IsNullOrWhiteSpace())
        {
            var swaggerRootUrl = configurationSection["SonEcommerce_Admin_Swagger:RootUrl"]?.TrimEnd('/');
